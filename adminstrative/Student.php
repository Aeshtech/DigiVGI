...
if (isset($_POST['submit'])) {
    ...
    $parentEmail = filter_var($parentEmail, FILTER_SANITIZE_EMAIL);
    ...
    if (!preg_match("/^[a-zA-Z- ']*$/", $name)) {
        ...
        $_SESSION['error'] = "Student Email should not be empty!!";
    } else if (filter_var($email, FILTER_VALIDATE_EMAIL) != TRUE) {
        $_SESSION['error'] = 'Please enter valid student email!';
    } else if (filter_var($parentEmail, FILTER_VALIDATE_EMAIL) != TRUE) {
        $_SESSION['error'] = 'Please enter valid parent email!';
        ...
        $query = "INSERT INTO `student`(`registration`,`name`,`phone`,`course`,`branch`,`semester`,`section`,`email`,`parentEmail`,`gender`,`password`,`photo`,`user_type`)VALUES('$registration','$name','$phone','$admin_course','$admin_branch','$semester','$section','$email','$parentEmail','$gender','$password','$upload','$user_type')";
...
if (isset($_POST['update'])) {
    ...
    if (!preg_match("/^[a-zA-Z- ']*$/", $name)) {
        ...
        $query = "UPDATE `student` SET `registration`=?,`name`=?,`phone`=?,`semester`=?,`section`=?,`email`=?,`parentEmail`=?,`gender`=?, `password`=?,`photo`=? WHERE `id`=? ";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ssssssssssi", $reg, $name, $phone, $semester, $section, $email, $parentEmail, $gender, $password, $upload, $id);
        ...
        <div class="grid-item2">
            ...
            <table class="List" id="studentList">
                ...
                <tbody>
                    ...
                    ?>
                            <tr>
                                <td><img src="<?php echo $row['photo'] ?>" style="width: 40px;height: 40px;border-radius: 50%;border: 2px solid var(--primary);">
                                </td>
        ...
        <div class="grid-item3 form">
            ...
            <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) ?>" method="post" enctype="multipart/form-data" autocomplete="off">
                <div>
                    ...
                    <input type="text" name="name" id="studentFullName" placeholder="-----Full Name-----" value="<?= $name; ?>" required>
                </div>
                <div>
                    <label>Registration No:</label>
                    <input type="text" maxlength="10" minlength="10" required name="registration" id="registrationNo" placeholder="-----Registration No-------" value="<?= $reg; ?>">
                </div>
                ...
                <div>
                    ...
                    <select name="section" required id="section">
                        <option value='None' selected <?php if ($section == 'None') {
                                                            echo "selected";
                                                        } ?>>None</option>
                        ...
                        <option value="C" <?php if ($section == 'C') {
                                                echo "selected";
                                            } ?>> C </option>
                    </select>
                </div>
                <div>
                    ...
                    <input type="text" name="email" required id="studentEmail" placeholder="-----Student Email----" value="<?= $email; ?>">
                </div>
                <div>
                    <label>Parent Email:</label>
                    <input type="text" name="parentEmail" required id="parentEmail" placeholder="-----Parent Email----" value="<?= $parentEmail; ?>">
                </div>
                <div>
                    <label>Contact No:</label>
                    <input type="text" required maxlength="10" minlength="10" name="phone" id="studentContactNo" placeholder="-----Contact No----" value="<?= $phone; ?>">
                </div>
                <div>
                    <label>Gender:</label>
                    <select name="gender" required="required" id="gender">
                        ...
                        <option value="not-defined" <?php if ($gender == 'not-defined') {
                                                        echo "selected";
                                                    } ?>> Not Defined
                        </option>
                    </select>
                ...
                <div>
                    ...
                    <input type="text" maxlength="12" minlength="4" name="password" id="studentPassword" placeholder="-----Password----">
